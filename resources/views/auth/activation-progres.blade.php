...
@section('title', 'Status Aktivasi Restaurant')

@section('content')

    <div x-data="activationStatus()" x-init="init()"
        ...
        <div class="relative z-10 max-w-sm w-full text-center">
            ...
            <h1 class="text-4xl font-black uppercase tracking-tighter italic leading-none mb-4">
                <template x-if="isSuccess">
                    <span>
                        Account <span :class="isDark ? 'text-emerald-500' : 'text-emerald-600'">Active</span>
                    </span>
                </template>
                <template x-if="!isSuccess">
                    <span>
                        Action <span :class="isDark ? 'text-red-500' : 'text-red-600'">Failed</span>
                    </span>
                </template>
            </h1>
            ...
            <p class="text-sm font-black uppercase tracking-[0.2em] mb-2 opacity-80"
                x-text="isSuccess ? 'Berhasil Diaktifkan' : 'Gagal Aktivasi'">
            </p>

            <div class="space-y-1 mb-10 px-4">
                <p class="text-xs font-medium leading-relaxed opacity-60 italic">
                    {{ $message }}
                </p>
                @if ($status === 'success')
            ...
            <div class="flex flex-col gap-3">
                <a href="{{ url('/login') }}"
                    ...
                    class="w-full py-4 rounded-2xl text-white text-xs font-bold uppercase tracking-widest transition-all duration-300 shadow-lg active:scale-95">
                    Masuk ke Dashboard
                </a>
            </div>
