...
<body class="transition-colors duration-500"
    ...
    <div class="flex h-dvh flex-col overflow-hidden transition-colors duration-500">
            ...
            <div class="flex items-center gap-2">
                <div class="-ml-5">
                    <button onclick="window.history.back()" class="group rounded-xl p-2 duration-200"
                        :class="isDark ? 'hover:bg-white/5 text-gray-400 hover:text-white' :
                    ...
                    <button onclick="window.history.forward()" class="group rounded-xl p-2 duration-200"
                        :class="isDark ? 'hover:bg-white/5 text-gray-400 hover:text-white' :
                            'hover:bg-gray-100 text-gray-600 hover:text-gray-900'">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                ...
                <div class="flex flex-col">
                    ...
                    @if ($pageTitle)
                        <span
                            class="text-[9px] font-black uppercase tracking-[0.2em] opacity-50">{{ $pageTitle }}</span>
                    @endif
                </div>
            </div>
                ...
                @if ($currentShift)
                    @if ($currentShift->id)
                        <div class="flex flex-col items-end border-r pr-4"
                            ...
                            <span class="text-[9px] font-bold uppercase tracking-tighter opacity-40">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <a href="/me"
                            class="flex h-10 w-10 items-center justify-center overflow-hidden rounded-2xl border-2"
                            :class="isDark ? 'bg-slate-800 border-white/10' : 'bg-gray-100 border-white shadow-sm'">
                            @if ($currentShift->user?->avatar_url)
                                <img src="{{ $currentShift->user->avatar_url }}" alt="avatar"
        ...
        <main class="relative h-full flex-1 pt-16">
            {{ $slot }}
        </main>
