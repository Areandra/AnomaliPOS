...
@section('content')
<div
    x-data="shiftIndex({{ json_encode($shifts->map(fn($s) => [
        'id'           => $s->id,
            ...
            <template x-for="s in filteredShifts" :key="s.id">
                <div :class="$store.theme.isDark
                    ...
                    <div class="flex items-center gap-3 mb-6">
                        <img :src="s.user.avatarUrl ?? `https://ui-avatars.com/api/?name=${encodeURIComponent(s.user.name)}&background=${$store.theme.isDark ? '1e293b' : 'f1f5f9'}&color=${$store.theme.isDark ? 'f59e0b' : 'ea580c'}&bold=true`"
                            :alt="s.user.name"
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-6">
                        <div :class="$store.theme.isDark ? 'bg-white/5' : 'bg-gray-50'" class="p-3 rounded-2xl">
                            ...
                            <p class="text-[11px] font-bold" x-text="formatRp(Number(s.cashSystem) + Number(s.startingCash))"></p>
                        </div>
                        <div :class="$store.theme.isDark ? 'bg-white/5' : 'bg-gray-50'" class="p-3 rounded-2xl">
                            <p class="text-[8px] font-black uppercase opacity-40 mb-1 flex items-center gap-1">
                                <i data-lucide="banknote" class="w-2.5 h-2.5"></i> Fisik
                            </p>
                            <p class="text-[11px] font-bold" x-text="s.cashPhysical ? formatRp(s.cashPhysical) : '---'"></p>
                        </div>
                    </div>
