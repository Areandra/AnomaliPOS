<x-admin-layout>
    <x-slot:title>{{ 'Detail Shift #' . $shift->id }}</x-slot:title>
    <x-slot:page_title>{{ 'Detail Shift #' . $shift->id }}</x-slot:page_title>

    @php
        $cashExpected = (float) $shift->modal_awal + (float) $shift->cash_system;

        // Siapkan payload data untuk AI Context
        $aiPayload = [
            'id' => $shift->id,
            'kasir' => $shift->user->name ?? 'Unknown',
            'status' => $shift->status,
            'waktu_buka' => $shift->opened_at,
            'keuangan' => [
                'modal_awal' => (float) $shift->modal_awal,
                'penjualan_sistem' => (float) $shift->cash_system,
                'harapan_kas' => $cashExpected,
                'fisik_kas' => (float) $shift->cash_physical,
                'selisih_variance' => (float) $shift->cash_variance,
            ],
            'digital_system' => [
                'qris' => (float) $shift->qris_system,
                'debit' => (float) $shift->debit_system,
                'transfer' => (float) $shift->transfer_system,
            ],
            'total_transaksi' => $shift->payments->count(),
        ];
    @endphp

    {{-- Inisialisasi Alpine.js untuk membungkus halaman ini --}}
    <div x-data="shiftDetailApp(@js($aiPayload))" class="flex h-full flex-col overflow-hidden">
        <x-receipt-modal />

        {{-- Header --}}
        <div :class="isDark ? 'border-white/5' : 'border-gray-100'" class="border-b p-5 md:p-8">
            ...
            <div class="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-center">
                <div class="flex items-center gap-4">
                    <div :class="isDark ? 'bg-slate-800 border-amber-500/20 text-amber-500' :
            ...
            <div class="grid max-w-[1600px] grid-cols-1 gap-6 md:gap-8 xl:grid-cols-12">
                ...
                <div class="order-1 space-y-6 md:space-y-8 xl:col-span-4">
                    ...
                    <section class="space-y-4">
                        ...
                        <div :class="isDark ? 'bg-slate-900/50 border-white/5' :
                            'bg-white border-gray-100 shadow-sm'"
                            class="rounded-[2rem] border p-6 md:rounded-[2.5rem]">
                            <div class="space-y-4">
                                <div class="flex items-center justify-between">
                                    <span
                                        class="text-[10px] font-bold uppercase tracking-widest opacity-40">Modal</span>
                                    <span class="text-sm font-black tracking-tight">Rp
                                        {{ number_format($shift->modal_awal, 0, ',', '.') }}</span>
                                </div>
                                <div class="flex items-center justify-between">
                                    <span
                                        class="text-[10px] font-bold uppercase tracking-widest opacity-40">Sales</span>
                                    <span class="text-sm font-black tracking-tight">Rp
                                        {{ number_format($shift->cash_system, 0, ',', '.') }}</span>
                                </div>
                                <div :class="isDark ? 'bg-white/5' : 'bg-gray-50'" class="h-px w-full">
                                </div>
                                <div class="flex items-center justify-between">
                                    <span
                                ...
                                <div class="flex items-center justify-between">
                                    <span
                                        class="text-[10px] font-bold uppercase tracking-widest opacity-40">Physical</span>
                    ...
                    {{-- Digital --}}
                    <section class="space-y-4">
                        <h3 class="ml-2 text-[9px] font-black uppercase tracking-[0.2em] opacity-40">Digital (System)
                        </h3>
                        <div class="grid grid-cols-1 gap-3 sm:grid-cols-3 xl:grid-cols-1">
                            @foreach ([['label' => 'QRIS', 'icon' => 'qr-code', 'value' => $shift->qris_system], ['label' => 'Debit', 'icon' => 'credit-card', 'value' => $shift->debit_system], ['label' => 'Transfer', 'icon' => 'wallet', 'value' => $shift->transfer_system]] as $d)
                                <div :class="isDark ? 'bg-slate-900/50 border-white/5 hover:bg-white/5' :
                                    'bg-white border-gray-100 hover:shadow-md'"
                                    class="flex items-center justify-between rounded-[2rem] border p-5 transition-all">
                                    <div class="flex items-center gap-3">
                                        <div :class="isDark ? 'bg-white/5 text-gray-400' :
                ...
                <div class="order-2 space-y-4 pb-10 xl:col-span-8 xl:pb-0">
                    <div class="ml-2 mt-4 flex items-center justify-between xl:mt-0">
                        ...
                        <span :class="isDark ? 'bg-white/5 text-amber-500' : 'bg-gray-100 text-orange-600'"
                            class="rounded-full px-3 py-1 text-[9px] font-black">
                            {{ $shift->payments->count() }} Items
                        </span>
                    </div>

                    <div :class="isDark ? 'bg-slate-900/50 border-white/5' : 'bg-white border-gray-100 shadow-sm'"
                        class="rounded-[2rem] border md:rounded-[2.5rem]">
                        <div class="overflow-x-auto">
                            <table class="w-full min-w-[500px] text-left">
                                <thead>
                                    <tr :class="isDark ? 'border-white/5' : 'border-gray-50'"
                                ...
                                <tbody>
                                    @forelse($shift->payments as $pay)
                                        <tr :class="isDark ? 'hover:bg-white/[0.02]' : 'hover:bg-gray-50/50'"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4">
                                                <div
                                                    class="flex flex-col items-center gap-4 py-20 text-center opacity-20">
                                                    <i data-lucide="receipt" class="h-12 w-12"></i>
                                                    <p class="text-[9px] font-black uppercase tracking-widest">No data
                                                    </p>
                                                </div>
                                            </td>
                                        </tr>
    ...
    <script>
        // Store untuk receipt modal tetap kita pertahankan di global init
        ...
        // Fungsi baru untuk Alpine Component membungkus detail shift
        function shiftDetailApp(shiftData) {
            return {
                shift: shiftData,

                askAI(module = 'shift_detail', customPayload = null) {
                ...
                init() {
                    ...
                    // Dengarkan event dari Floating Button Component
