<x-cashier-layout :currentShift="$currentShift" pageTitle="Point of Sale">
    ...
    <div x-data="cashierApp(
        ...
        {{-- Open Shift Panel --}}
        <template x-if="!shift">
            @include('components.open-shift-panel')
        </template>

        {{-- Close Shift Modal --}}
        <template x-if="openCloseModal">
            @include('components.close-shift-modal')
        </template>

        {{-- Active Order Panel --}}
        @include('components.active-order-panel')
    ...
    return {
        ...
        get cartInfoMap() {
            ...
            (this.selectedOrder?.items || []).forEach(item => {
                ...
                if (!map[menuId]) map[menuId] = { totalQty: 0, cartItem: null };
        ...
        async fetchOrder(id) {
            if (!id) { this.selectedOrder = null; return; }
            try {
                const res = await fetch(`/api/order/${id}`, {
                    headers: { 'X-Requested-With': 'XMLHttpRequest' }
                });
                this.selectedOrder = await res.json();
            } catch(e) { console.error('Gagal fetch order'); }
        },

        async handleAddOrUpdate(item) {
            if (!this.selectedOrderId) return alert('Silahkan pilih nomor meja/order terlebih dahulu!');
            const info = this.cartInfoMap[item.id] || { totalQty: 0, cartItem: null };
            try {
                let res;
                if (info.cartItem) {
                    res = await this.post('/api/order/update-qty', { itemId: info.cartItem.id, qty: info.cartItem.quantity + 1 });
                } else {
                    res = await this.post('/api/order/add-item', { orderId: this.selectedOrderId, menuItemId: item.id, qty: 1 });
                }
                this.selectedOrder = await res.json();
            } catch(e) { alert('Gagal memproses item'); }
        },
        ...
        async updateItemQty(itemId, qty) {
            try {
                let res;
                if (qty < 1) {
                    res = await this.post('/api/order/delete-item', { itemId });
                } else {
                    res = await this.post('/api/order/update-qty', { itemId, qty });
                }
                this.selectedOrder = await res.json();
            } catch(e) { alert('Gagal update quantity'); }
        },

        async orderAll(orderId) {
            try {
                const res = await this.post(`/api/order/place-order/${orderId}`, {});
                this.selectedOrder = await res.json();
            } catch(e) { alert('Gagal kirim order ke dapur'); }
        },
        ...
        async newOrder() {
            if (this.atLeastPro) {
                ...
                const res = await this.post('/api/order', {});
        ...
        async handleOpenShift() {
            ...
            try {
                const res = await fetch('/api/shifts/open', {
                    ...
                    body: JSON.stringify({ startingCash: Number(this.startingCash) })
                });
...
function closeShiftModal() {
    return {
        ...
        closedUser: null,
        currencies: [
            { value: 100000, label: '100.000', gradient: 'from-red-600 via-red-500 to-red-700', isCoin: false },
            { value: 75000,  label: '75.000',  gradient: 'from-red-400 via-pink-400 to-red-500', isCoin: false },
            { value: 50000,  label: '50.000',  gradient: 'from-blue-600 via-blue-500 to-blue-700', isCoin: false },
            { value: 20000,  label: '20.000',  gradient: 'from-green-600 via-green-500 to-green-700', isCoin: false },
            { value: 10000,  label: '10.000',  gradient: 'from-purple-600 via-purple-500 to-purple-700', isCoin: false },
            { value: 5000,   label: '5.000',   gradient: 'from-yellow-600 via-yellow-500 to-yellow-700', isCoin: false },
            { value: 2000,   label: '2.000',   gradient: 'from-slate-500 via-slate-400 to-slate-600', isCoin: false },
            { value: 1000,   label: '1.000',   gradient: 'from-lime-600 via-lime-500 to-lime-700', isCoin: false },
            { value: 500,    label: '500',     gradient: 'from-zinc-400 via-zinc-300 to-zinc-500', isCoin: true },
            { value: 200,    label: '200',     gradient: 'from-zinc-300 via-zinc-200 to-zinc-400', isCoin: true },
        ],

        get totalPhysical() {
            return Object.entries(this.counts).reduce((acc, [val, count]) => acc + Number(val) * (Number(count) || 0), 0);
        },

        handleNumpad(num) {
            const current = this.counts[this.activeValue] || '';
            if (current.length >= 4) return;
            this.counts = { ...this.counts, [this.activeValue]: (current === '0' ? '' : current) + num };
        },
        handleClear() { this.counts = { ...this.counts, [this.activeValue]: '0' }; },
        handleBackspace() {
            const current = this.counts[this.activeValue] || '';
            this.counts = { ...this.counts, [this.activeValue]: current.length > 1 ? current.slice(0, -1) : '0' };
        },
        ...
        async handleCloseShift() {
            ...
            try {
                const res = await fetch('/api/shifts/close', {
                    ...
                    body: JSON.stringify({ cashPhysical: this.totalPhysical, notes: this.notes })
                });
                const data = await res.json();
                if (!res.ok) throw new Error(data.message);
...
function orderDetailPanel() {
    return {
        ...
        async saveNote() {
            ...
            const res = await fetch(`/api/order/${this.orderNoteItemId}/notes`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
