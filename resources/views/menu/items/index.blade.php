...
@section('content')
<div x-data="menuIndex({{ json_encode($items->map(fn($i) => [
    'id'          => $i->id,
    'name'        => $i->name,
...
])->values()) }}, {{ json_encode($categories->map(fn($c) => [
    'id'        => $c->id,
    'name'      => $c->name,
    ...
    <aside :class="$store.theme.isDark ? 'bg-slate-900 border-white/5' : 'bg-white border-gray-200'"
        ...
        <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
            <div @click="categorySelect = -1"
                ...
                class="flex items-center px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 cursor-pointer">
                All Categories
            </div>
            <template x-for="(cat, idx) in sortedCategories" :key="cat.id">
                <div :class="categorySelect === cat.id
                    ...
                    <p @click="categorySelect = cat.id" class="flex-1 truncate py-1 cursor-pointer" x-text="cat.name"></p>
            ...
            <template x-for="cat in groupedMenu" :key="cat.id">
                <div class="mb-6">
                    <h2 :class="$store.theme.isDark ? 'text-amber-500' : 'text-orange-600'"
                        ...
                        <span :class="$store.theme.isDark ? 'bg-amber-500' : 'bg-orange-600'" class="h-1.5 w-1.5 rounded-full"></span>
                        ...
                        <template x-for="item in cat.items" :key="item.id">
                            <div :class="$store.theme.isDark ? 'bg-slate-900/50 border-white/5 hover:bg-slate-800 shadow-2xl shadow-black/20' : 'bg-white border-gray-100 hover:shadow-xl shadow-gray-200/50'"
                                ...
                                <div class="h-44 w-full overflow-hidden relative">
                                    ...
                                    <div class="absolute inset-0 bg-gradient-to-t from-slate-950/60 to-transparent opacity-60"></div>
                                </div>

                                <div class="p-6 flex-1 flex flex-col">
                                    <div class="mb-4">
                                        <div class="flex items-center gap-2 mb-1">
                                            <i data-lucide="tag" :class="$store.theme.isDark ? 'text-amber-500' : 'text-orange-600'" class="w-2.5 h-2.5"></i>
                                    ...
                                    <div class="mt-auto">
                                        <div class="flex items-baseline gap-1 mb-5">
                                            <span :class="$store.theme.isDark ? 'text-gray-400' : 'text-gray-500'" class="text-[10px] font-bold opacity-50">Rp</span>
                                        ...
                                        <div class="grid grid-cols-2 gap-2">
                                            <button @click="toggleAvailability(item)"
                                                ...
                                                <template x-if="item.isAvailable"><i data-lucide="eye-off" class="w-3 h-3"></i></template>
                                                <template x-if="!item.isAvailable"><i data-lucide="eye" class="w-3 h-3"></i></template>
