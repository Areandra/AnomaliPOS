...
@section('content')
<div x-data="menuCreate()" x-init="$store.theme.init(); $nextTick(() => lucide.createIcons())" class="">
        ...
        <div :class="$store.theme.isDark ? 'bg-slate-900 border-white/5' : 'bg-white border-gray-100'"
            ...
            <div :class="$store.theme.isDark ? 'border-white/5 bg-white/5' : 'border-gray-50 bg-gray-50/50'"
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 :class="$store.theme.isDark ? 'text-white' : 'text-slate-800'" class="text-xl font-black uppercase tracking-tight">Register Menu Item</h2>
                        ...
                        <div class="relative group">
                            ...
                            <div class="relative">
                                ...
                                <textarea name="description" rows="3"
                                    :class="$store.theme.isDark ? 'bg-slate-950 border-white/5 text-white' : 'bg-gray-50 border-gray-200'"
                                    class="w-full pl-12 pr-4 py-4 rounded-2xl border text-sm font-bold transition-all outline-none min-h-30"
                    ...
                    <div class="md:col-span-5 space-y-8">
                        <div :class="$store.theme.isDark ? 'bg-white/5 border-white/10' : 'bg-gray-50 border-gray-200'"
                            ...
                            <div class="flex items-center gap-2 mb-6">
                                ...
                                <span :class="$store.theme.isDark ? 'text-white' : 'text-slate-800'" class="text-[10px] font-black uppercase tracking-widest">Finance & Pricing</span>
                            </div>
                            <div class="space-y-6">
                                <div>
                                    <label class="block text-[9px] font-black uppercase tracking-widest mb-2 opacity-50">Selling Price</label>
                                    <input type="number" name="price" value="{{ old('price', 0) }}" min="0" step="100" required
                                ...
                                <div>
                                    ...
                                    <input type="number" name="cost_of_goods" value="{{ old('cost_of_goods', 0) }}" min="0" step="100"
                                        :class="$store.theme.isDark ? 'bg-slate-950 border-white/5 text-white' : 'bg-white border-gray-200 text-slate-900'"
                                        class="w-full px-5 py-4 rounded-xl border text-lg font-black tracking-tighter outline-none" />
                                </div>
                            </div>
                        ...
                        <div class="space-y-4">
                            <label class="block text-[10px] font-black uppercase tracking-[0.2em] mb-3 opacity-50">Item Media</label>
                            <div class="relative">
                ...
                <div :class="$store.theme.isDark ? 'border-white/5' : 'border-gray-100'"
                    ...
                    <div class="flex items-center gap-6">
                        <label class="flex items-center gap-2 cursor-pointer">
                            ...
                            <span :class="$store.theme.isDark ? 'bg-emerald-500/10 border-emerald-500/50 text-emerald-400' : 'bg-emerald-50 border-emerald-200 text-emerald-700'"
                                class="flex items-center gap-2 px-5 py-3 rounded-xl border text-[10px] font-black uppercase tracking-widest">
                                <i data-lucide="check-circle-2" class="w-4 h-4"></i> In Stock
                            </span>
                        </label>
                    </div>
                    <div class="flex items-center gap-4">
                        ...
                        <button type="submit"
                            ...
                            <i data-lucide="save" class="w-3.5 h-3.5"></i> Register Item
                        </button>
                    </div>
                </div>
            </form>
...
<script>function menuCreate() { return {} }</script>
@endsection
